...
sink("vPart1_Demonstration_DecSpace_2.txt", append=TRUE)
cat("nrep, nTarget, nAppLoc, MissionTimeLimit:, TotalTimeTour:, wlimit, Status, Optimality gap, Run Time given by Gurobi, Run Time given by R, Objective value, teta_result, Best available upper bound, TotalTimeTour, x_ijkl_variables, yia_variables, sia_results, sia_variables")
cat("\n")
sink()

sink("vPart1_Demonstration_Summary_2.txt", append=TRUE)
cat("nrep, nTarget, nAppLoc, MissionTimeLimit, TotalTimeTour, wlimit, TotalRadarThreat, result$status, result$objval, result$objbound, result$mipgap, result$runtime, runTime")
cat("\n")
sink()
...
PayloadRangeMaxMinLoadQuant<-matrix(c(14,25,25,3,5,5,125,250,325,6,3,2),nrow=nPayloadType,ncol=4) 

R<-max(PayloadRangeMaxMinLoadQuant[,1]) 

for(nrep in 12:repCount){
  set.seed(SeedAll[nrep])
  nrep
  for (nTarget in nTargetSet){
    nNodes<- nTarget + 1 
    ...
    #CONSTRAINT CONSTRUCTION
            ...
            #Constraint 10 - At most Q quantities can be used for each target
              ...
              C10_rhs<-c(PayloadRangeMaxMinLoadQuant[,4])
          ...
          for(MissionTimeLimit in MissionTimeLimitSet){
              wlimit<-wSet[which(MissionTimeLimitSet==MissionTimeLimit)]
    ...
    # BUILD MODEL
              ...
              model <- list()
              model$modelname <- 'UCAV Expected Profit'
        ...
        # gurobi_write(model, 'C:/Users/..../UCAV.mps')
              ...
              #all_names<-c(X_ijkl_names,Y_ia_names,S_ia_names,P_ia_names, u_ij_names)
              ...
              #ResultReports
              cat(c(nrep, nTarget, nAppLoc, MissionTimeLimit, TotalTimeTour,TotalRadarThreat,result$status, result$objval, result$objbound,result$mipgap,result$runtime, runTime), file = "v24_2_Demonstration_Summary.txt", append = TRUE)
              cat("\n", file ="vPart1_Demonstration_Summary.txt", append = TRUE)
              
              sink("vPart1_Demonstration_DecSpace.txt", append=TRUE)
              
              cat("Problem Parameters (rep, NumberofTargets, NumberofWaypoints, MissionTimeLimit:, TotalTimeTour:", c(nrep, nTarget, nAppLoc, MissionTimeLimit,TotalTimeTour))
              cat(",Status, Optimality gap, Run Time given by Gurobi, Run Time given by R, Objective value, teta_result, Best available upper bound, TotalTimeTour, x_ijkl_variables, yia_variables, sia_results, sia_variables",c(result$status,(result$mipgap)*100,result$runtime,runTime,result$objval,teta_result,result$objbound,TotalTimeTour,x_ijkl_variables,yia_variables,sia_results,sia_variables))
              cat("\n")
              ...
              sink("vPart1_Demonstration_Summary_2.txt", append=TRUE)
              cat(c(nrep, nTarget, nAppLoc, MissionTimeLimit, TotalTimeTour, wlimit, TotalRadarThreat, result$status, result$objval, result$objbound, result$mipgap, result$runtime, runTime))
              cat("\n")
              sink()
              
              sink("vPart1_Demonstration_DecSpace_2.txt", append=TRUE)
              cat(c(nrep, nTarget, nAppLoc, MissionTimeLimit,TotalTimeTour, wlimit, result$status,(result$mipgap)*100,result$runtime,runTime,result$objval,teta_result,result$objbound,TotalTimeTour,x_ijkl_variables,yia_variables,sia_results,sia_variables))
              cat("\n")
              sink()
